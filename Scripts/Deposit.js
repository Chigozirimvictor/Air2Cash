...
function fetchSecretKeyAndPay () {
  ...
  $.ajax({
    type: 'post',
    url: 'https://testing1-xpjd.onrender.com/api/deposits/getKey', // replace wi=th your actual logout URL
    headers: {
...
function save (reference, amount) {
  ...
  if (!id || !reference || !amount || !token) {
    ...
    showSweetAlert(
      'User ID, reference, amount, and token are required for this transaction',
      false
    )
    return
  }
